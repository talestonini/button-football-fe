...
package object model:
  ...
  val sNumTeams: Signal[Int] = sGroups.map(gm => gm.length * NUM_TEAMS_PER_GROUP)
  val sNumFinalsMatches: Signal[Int] = sFinalsMatches.map(fms => fms.length)
  val sTabs: Signal[List[String]] = sGroups.map(gs => gs :+ FINALS_TAB :+ FINAL_STANDINGS_TAB :+ RANKING_TAB)
